...
export function getMinMax(period: Period, periods: Period[]): MinMax {
  ...
  return { min, max, previous, next };
}

export type PeriodsEquals = {
  startEqual: boolean;
  stopEqual: boolean;
  isModified: boolean;
};

export function getPeriodsEquals(period: Period, altered: Period): PeriodsEquals {
  ...
  return { isModified, startEqual, stopEqual };
}
...
export const getDifferences = (
  ...
  altered: Period,
  minMax: MinMax,
  equals: PeriodsEquals,
  findTime: (when: number) => Time | undefined,
): { deleted: number[]; inserted: Time[] } => {
  const { previous, next } = minMax;
  const { startEqual, stopEqual } = equals;
